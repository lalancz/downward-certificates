...
size_t CertificateManager::apply_dead_rule(size_t setid, std::string rulename,
                                           std::vector<size_t> justification) {
    int new_kid = get_new_knowledgeid();
...
size_t CertificateManager::apply_subset_rule(size_t left_setid, size_t right_setid, std::string rulename,
                                             std::vector<size_t> justification) {
    int new_kid = get_new_knowledgeid();
...
Judgment CertificateManager::apply_rule_ed() {
    return empty_dead;
}
Judgment CertificateManager::apply_rule_ud(const SetExpression &set, const Judgment &s_dead,
                                           const Judgment &sp_dead) {
    return Judgment(apply_dead_rule(set.id, "ud", {s_dead.id, sp_dead.id}));
}
Judgment CertificateManager::apply_rule_sd(const SetExpression &set, const Judgment &sp_dead,
                                           const Judgment &s_subset_sp) {
    return Judgment(apply_dead_rule(set.id, "sd", {sp_dead.id, s_subset_sp.id}));
}
Judgment CertificateManager::apply_rule_pg(const SetExpression &set, const Judgment &progression_subset,
                                           const Judgment &sp_dead,
...
Judgment CertificateManager::apply_rule_pi(const SetExpression &set, const Judgment &progression_subset,
                                           const Judgment &sp_dead,
                                           const Judgment &init_subset) {
    return Judgment(apply_dead_rule(set.id, "pi", {progression_subset.id, sp_dead.id, init_subset.id}));
}
Judgment CertificateManager::apply_rule_rg(const SetExpression &set, const Judgment &regression_subset,
                                           const Judgment &sp_dead,
                                           const Judgment &goal_intersection_dead) {
    return Judgment(apply_dead_rule(set.id, "rg", {regression_subset.id, sp_dead.id, goal_intersection_dead.id}));
}
Judgment CertificateManager::apply_rule_ri(const SetExpression &set, const Judgment &regression_subset,
                                           const Judgment &sp_dead,
                                           const Judgment &init_subset) {
    return Judgment(apply_dead_rule(set.id, "ri", {regression_subset.id, sp_dead.id, init_subset.id}));
}

Judgment CertificateManager::apply_rule_ci(const Judgment &init_dead) {
    int new_kid = get_new_knowledgeid();
    certstream << "k " << new_kid << " u ci " << init_dead.id << "\n";
    return Judgment(new_kid);
}
Judgment CertificateManager::apply_rule_cg(const Judgment &goal_dead) {
    int new_kid = get_new_knowledgeid();
